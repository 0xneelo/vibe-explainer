...
const INTRO_CAPTIONS = [
  { t: 0.6, t2: 5.0, text: 'Introducing the Permissionless Credit Layer, build on Hyperliquid.' },
  { t: 5.7, t2: 8.3, text: 'Orderbooks scale markets. Vibe Trading helps discover which assets deserve leverage.\n\n\nLet me explain how Credit is the missing piece to house all of finance.' },
];
...
// Scene-local geometry for "Render" (vibe-render.jsx): each scene's start, the
// total scene-local length, and each caption line's scene-local cue (intro lines
// are already scene-local; S1–S9 lines shift by INTRO_SCENE). Render measures the
// real clip durations and warps real↔scene-local so scenes match the voiceover.
const VIBE_SCENE_STARTS = VIBE_SCENE_NAMES.map((s) => s[0]);
const VIBE_TOTAL_SCENE = 212 + INTRO_SCENE;
const VIBE_LINE_CUES = INTRO_CAPTIONS.map((c) => c.t)
  .concat(VIBE_CAPTIONS.map((c) => c.t + INTRO_SCENE));
const VIBE_RENDER_KEY = 'vibe-render-durations-v1';
...
function VibeMovie({ items, captionsOn, voiceoverOn, speed, engine, elVoice, elKey,
  ...
  // Scene-local clock: ×SLIDE_SLOW by default, or the Render warp when fitted.
  const st = renderMap ? renderMap.realToScene(t) : t / SLIDE_SLOW;
  ...
  return (
    <div data-screen-label={`${Math.floor(t)}s · ${scene ? scene[1] : ''}`}
      ...
      <TimeScale factor={SLIDE_SLOW}
        map={renderMap ? renderMap.realToScene : null}
        inv={renderMap ? renderMap.sceneToReal : null}>
...
// Small always-available corner controls: open the slides/transcript panels
// and the Tweaks console without needing the host toolbar toggle.
function CornerControls({ transcriptOn, onTranscript, slidesOn, onSlides, hidden,
  rendered, rendering, onRender, onClearRender, children }) {
  ...
  const renderLabel = rendering ? rendering : rendered ? 'Rendered ✓' : 'Render';
  return (
    <div style={{ position: 'absolute', top: 12, right: 14, zIndex: 60,
      display: hidden ? 'none' : 'flex', gap: 8 }}>
      {children}
      <button style={btn(!!rendered)} disabled={rendering != null}
        title={rendered
          ? 'Scenes are paced to the narration — click to revert to default pacing'
          : 'Fit each scene to its voiceover (measures the generated clips)'}
        onClick={() => (rendered ? onClearRender() : onRender())}>{renderLabel}</button>
      <button style={btn(slidesOn)} onClick={() => onSlides(!slidesOn)}>Slides</button>
...
// ?volume=20 → start master volume (narration + music) at 20%. Returns a number
// clamped to 0–100, or null when the param is absent/invalid (use the stored
// value then). Accepts ?volume= as a 0–100 percentage.
function vibeUrlVolume() {
  try {
    const raw = new URLSearchParams(window.location.search).get('volume');
...
function VibeApp() {
  ...
  // Master volume 0–100 (narration + music). A ?volume= URL param (0–100) wins
  // on load — e.g. ?volume=20 plays everything at 20% — otherwise the value
  // persisted per browser is used.
  const [volume, setVolume] = React.useState(() => {
    const urlVol = vibeUrlVolume();
  ...
  // ── Render: pace scenes to the real narration (vibe-render.jsx) ───────────
  // renderMap warps real↔scene-local so each scene spans its lines' clip audio;
  // null = default ×SLIDE_SLOW pacing. The measured durations persist so the
  // fit survives reloads; "Render" re-measures, clicking again clears it.
  const [renderMap, setRenderMap] = React.useState(null);
  const [rendering, setRendering] = React.useState(null); // status string | null
  const buildRenderMap = (durations) => vibeBuildSceneRenderMap({
    durations, lineCues: VIBE_LINE_CUES,
    sceneStarts: VIBE_SCENE_STARTS, totalScene: VIBE_TOTAL_SCENE,
  });
  React.useEffect(() => {
    try {
      const d = JSON.parse(localStorage.getItem(VIBE_RENDER_KEY) || 'null');
      if (Array.isArray(d) && d.length === VIBE_LINE_CUES.length) setRenderMap(buildRenderMap(d));
    } catch {}
  }, []);
  // ── Slide text (on-screen frame text, edited via the "Slides" panel) ──────
  const [slidesOn, setSlidesOn] = React.useState(false);
  // Subscribe to the slide-text store: any edit/import/reset re-renders the
  // stage so ST(...) calls in the scenes pick up the new strings immediately.
  useSlideTexts();
  // Published baseline: assets/slide-text.json ships with every deploy; a
  // local browser draft still wins (same layering as the transcript).
  React.useEffect(() => {
    let alive = true;
    vibeFetchPublishedSlideText().then((texts) => {
      if (alive && texts) stSetPublished(texts);
    });
    return () => { alive = false; };
  }, []);
  // Panel scene groups: label + start in REAL timeline seconds (scene clocks
  // run ×SLIDE_SLOW; seekTo's 0.2s pre-roll handles the write-on re-trigger).
  const slideScenes = VIBE_SCENE_NAMES.map(([tt, label], i) => ({
    id: `s${i}`, label,
    t: renderMap ? Math.round(renderMap.sceneRealStarts[i] * 10) / 10
      : Math.round(tt * SLIDE_SLOW * 10) / 10,
  }));
  ...
  // Active caption cues (Render warp or default), read live by the bridge.
  const cuesRef = React.useRef(VIBE_CAPTIONS_REAL);
  const onTlCtx = React.useCallback((ctx) => {
    tlRef.current = ctx;
    window.__vibeTl = ctx; // dev/verifier hook: seek via __vibeTl.setTime(s)
    const time = ctx.time;
    const cues = cuesRef.current || VIBE_CAPTIONS_REAL;
    let idx = -1;
    for (let i = 0; i < cues.length; i++) {
      if (time >= cues[i].t && time < cues[i].t2) { idx = i; break; }
      if (time >= cues[i].t) idx = i;
    }
  ...
  // Render: measure the current narration clips and re-pace the scenes to them.
  const doRender = async () => {
    setRendering('Measuring narration…');
    try {
      const durations = await vibeMeasureDurations(items, elVoiceId(t.elVoice));
      setRenderMap(buildRenderMap(durations));
      try { localStorage.setItem(VIBE_RENDER_KEY, JSON.stringify(durations)); } catch {}
      setRendering(null);
    } catch (e) {
      setRendering('⚠ ' + (e && e.message ? e.message : e));
    }
  };
  const clearRender = () => {
    setRenderMap(null);
    setRendering(null);
    try { localStorage.removeItem(VIBE_RENDER_KEY); } catch {}
  };
  // Active timing: Render warp when fitted, else the authored ×SLIDE_SLOW cues.
  const playItems = React.useMemo(() => (
    renderMap ? items.map((c, i) => (renderMap.cues[i]
      ? { ...c, t: renderMap.cues[i].t, t2: renderMap.cues[i].t2 } : c)) : items
  ), [items, renderMap]);
  const playCues = renderMap ? renderMap.cues : VIBE_CAPTIONS_REAL;
  cuesRef.current = playCues;
  const duration = renderMap ? renderMap.duration : VIBE_DURATION;
  const chapters = renderMap
    ? VIBE_SCENE_NAMES.map((s, i) => ({ t: Math.round(renderMap.sceneRealStarts[i] * 10) / 10, label: s[1] }))
    : VIBE_CHAPTERS;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', background: '#0a0a0a', ...vars }}>
      <div style={{ flex: 1, position: 'relative', minWidth: 0 }}>
        <Stage width={1920} height={1080} duration={duration} background="var(--paper)"
          speed={t.speed} autoplay={VIBE_EMBED} persistKey="vibe-explainer"
          chapters={chapters} bar={!recMode && !VIBE_EMBED} loop={false}
          ...
          <VibeMovie items={playItems} captionsOn={t.captions} voiceoverOn={t.voiceover}
            ...
            muted={muted} volume={volume / 100} admin={admin}
            renderMap={renderMap}></VibeMovie>
        ...
        {admin && !VIBE_EMBED && (
          <CornerControls transcriptOn={t.transcript} hidden={recMode}
            ...
            slidesOn={slidesOn} onSlides={setSlidesOn}
            rendered={!!renderMap} rendering={rendering}
            onRender={doRender} onClearRender={clearRender}>
      ...
      {admin && slidesOn && !recMode && (
        <SlideTextPanel scenes={slideScenes} onSeek={seekTo}
          onReset={stReset} dirty={stDirty()}></SlideTextPanel>
      )}
      {admin && t.transcript && !recMode && (
        <TranscriptPanel items={playItems} activeIdx={activeIdx}
          voiceLabel={t.elVoice} apiKey={elKey}
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Engine" value={t.engine} options={['elevenlabs', 'mp3 file']}
